...
page = A.page[pageID] = {
    ...
    page.items.body.find('#alert').html('');
		...
		*/
		...
		page.items.deck.html(h).find('li').on('click', function(){
			...
			if(!page.items.active){
              ...
              page.items.body.find('#alert').html('Well done, You have won the game in ' + page.items.moves + ' moves... and in a time of ' + page.items.timerVal[0]  + ':' + page.items.timerVal[1] + ' with a star rating of ' + page.items.body.find('ul.stars li:visible').length +'... click restart to play again!!<br><input type="button" value="restart" />').find('input').on('click', function(){
